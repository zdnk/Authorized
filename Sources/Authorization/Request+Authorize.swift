...
extension Request {
    ...
    public func authorize<A, R>(_: A.Type, _ resource: R, _ action: R.Action) throws where A: Authorizable & Authenticatable, R: Protected {
    ...
    public func authorize<A, R>(_: A.Type, _ resource: R.Type, _ action: R.Action) throws where A: Authorizable & Authenticatable, R: Protected {
        let user = try self.requireAuthenticated(A.self)
        let permissions = try self.make(Permissions.self)
